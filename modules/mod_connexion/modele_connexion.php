...
class ModeleConnexions extends Connexion {
    ...
    function verifierUtilisateur($email, $password) {
        ...
        $query = $this->connexion->getBdd()->prepare('SELECT id_joueur, Nom_joueur, mot_de_passe FROM joueur WHERE email = :email');
        ...
        if ($result && password_verify($password, $result['mot_de_passe'])) {
            ...
            return array(true, $result['id_joueur'], $result['Nom_joueur']);
        } else {
